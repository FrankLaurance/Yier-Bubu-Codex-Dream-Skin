...
- 内置「一二布布 · 暖暖编码」默认预设与 5 套可复现的程序化抽象预设；安装后幂等播种到主题库，菜单栏或 `switch-theme-macos.sh` 可直接切换，且绝不覆盖 `custom-*` 用户主题。
...
- 主题切换先完整暂存图片，最后原子发布 `theme.json`；全新安装在没有活动主题时先启用「一二布布 · 暖暖编码」，已有活动主题保持不变。
- `load-image-theme-macos.sh` 可通过 `--appearance`、`--focus-x`、`--focus-y`、`--safe-area`、`--task-mode` 精确调节构图；旧主题缺省时使用安全自适应值。

### 修复

- 兼容当前 Codex 把 `home-icon` 放到首页主区之外的结构：用标题与建议卡作为稳定回退标记，并直接将建议卡徽章和 SVG 按双轴居中，不再统一偏左；加入跨平台结构与样式回归检查。
...
- 一二布布生成源图、标准化背景和概念效果图分别归档；概念图只作预览，不能当背景导入。仓库随附的项目原创主题图像与源码统一采用 MIT License。
...
- 上游旧版曾提供独立纯横幅示例；本主题版统一使用 16:9 一二布布纯背景。
