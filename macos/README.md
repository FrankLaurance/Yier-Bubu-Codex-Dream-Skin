...
# 3) Switch to the bundled Yier & Bubu preset, or import your own pure background
~/.codex/codex-dream-skin-studio/scripts/switch-theme-macos.sh --id preset-yier-bubu-cozy
...
A fresh install seeds the custom Yier & Bubu preset plus five procedural abstract
presets into your theme library. **一二布布 · 暖暖编码** is selected by default:

```bash
~/.codex/codex-dream-skin-studio/scripts/switch-theme-macos.sh --id preset-yier-bubu-cozy
```

The generated source PNG is `1672 × 941`. Its pack contains a standardized
`2560 × 1440` JPEG plus theme metadata; the export does not add source detail.
The source art is archived at
[`docs/images/presets/yier-bubu-wallpaper-source.png`](../docs/images/presets/yier-bubu-wallpaper-source.png).
[`yier-bubu-concept-preview.png`](../docs/images/presets/yier-bubu-concept-preview.png)
contains an interface mockup and is documentation only — never import it as a
background. The UI-free release wallpaper is
[`yier-bubu-wallpaper-2560x1440.jpg`](../docs/images/presets/yier-bubu-wallpaper-2560x1440.jpg).
The artwork is not an official OpenAI/Codex visual or endorsement. Bundled
project-created artwork is released under the repository MIT License.
...
starts from **Yier & Bubu · Cozy Coding**; it never overwrites an
...
runtime Node, user-provided artwork, and third-party rights that the project
cannot license.
