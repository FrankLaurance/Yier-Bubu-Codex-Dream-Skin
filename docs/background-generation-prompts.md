...
| 你要做的事 | 应看的文件 | 说明 |
|---|---|---|
| 直接切换当前默认主题 | `macos/presets/preset-yier-bubu-cozy/theme.json` + `background.jpg`；Windows 对应 `windows/assets/theme.json` + `dream-reference.jpg` | 两端都播种「一二布布 · 暖暖编码」 |
...
| 查看一二布布目标效果 | `docs/images/presets/yier-bubu-concept-preview.png` | 概念预览，包含界面，不能作为背景素材 |

特别注意：`skin-01`～`skin-08` 只提供灵感；实际预设以 `macos/presets/preset-*` 目录中的 `theme.json` + `background.jpg` 为准。一二布布概念图也不是运行时背景。任何 `banner-*.png` / `generated-*.png` 实验图都不自动获得预设或发行资格；角色素材和模型输出必须先完成权利核验。
...
- 一二布布生成源图：`docs/images/presets/yier-bubu-wallpaper-source.png`（`1672 × 941` PNG）
- macOS 默认可切换背景：`macos/presets/preset-yier-bubu-cozy/background.jpg`（`2560 × 1440` JPEG）
- 一二布布概念效果图：`docs/images/presets/yier-bubu-concept-preview.png`（包含 UI，仅预览，禁止当背景导入）
