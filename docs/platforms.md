...
- Windows 安装会把运行所需的 `assets/` 与 `scripts/` 原子复制到 `%LOCALAPPDATA%\CodexDreamSkin\engine`，所有快捷方式均指向该受管副本，因此安装后可移动或删除源码目录。安装还会初始化 `active-theme`、`themes` 与 `images`，并把「一二布布 · 暖暖编码」同时设为首次默认和可切换的已保存主题。系统托盘支持更换背景、保存当前主题、从「已保存主题」切换、暂停和恢复；图片与 `theme.json` 保存在主题仓库中，不写进 Codex 的 `config.toml`。安装会保留用户已有的 `appearanceTheme`；仅在识别到旧版精确托管的浅色三元组时按备份迁移。
- Windows 渲染器仍支持在注入前用 `window.__CODEX_DREAM_SKIN_CONFIG__` 提供内存级可选覆盖（形状同上，颜色覆盖使用 `palette.accent`），但普通用户应优先使用持久化主题仓库与托盘。

## 预设与图片类型

- `macos/presets/preset-yier-bubu-cozy/` 是本项目的默认主题「一二布布 · 暖暖编码」。macOS 安装后可用 `switch-theme-macos.sh --id preset-yier-bubu-cozy` 切换。
- preset pack 中只有 `background.jpg`（`2560 × 1440`、16:9、纯背景）和 `theme.json` 会被播种。生成源图位于 `docs/images/presets/yier-bubu-wallpaper-source.png`；可导入的发行副本为 `yier-bubu-wallpaper-2560x1440.jpg`。`yier-bubu-concept-preview.png` 包含界面概念，只作文档预览，不能导入为背景。
- `preset-midnight-aurora` 等五套是从上游保留的程序化抽象示例。一二布布主题图与源码统一采用仓库 MIT License，但不代表 OpenAI/Codex 或任何第三方的官方视觉与背书。
- Windows 自带与 macOS byte-identical 的 `windows/assets/dream-reference.jpg` 纯背景，并在首次初始化时播种「一二布布 · 暖暖编码」。
