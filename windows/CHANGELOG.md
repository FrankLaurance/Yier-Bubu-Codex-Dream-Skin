...
- Windows 首次安装会把 UI-free 的 2560 × 1440「一二布布 · 暖暖编码」设为活动主题并播种到「已保存主题」，无需再从 macOS 目录手动导入。

### 修复

- 兼容当前 Codex 把 `home-icon` 放到首页主区之外的结构：用标题与建议卡作为稳定回退标记，并直接将建议卡徽章和 SVG 按双轴居中，不再统一偏左；加入与 macOS 对等的结构与样式回归检查。
