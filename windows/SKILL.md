...
description: Apply, launch, verify, repair, update, or restore the Yier & Bubu decorative skin for the Windows Codex desktop app without modifying WindowsApps or app.asar.
---
...
1. Install Node.js 22 or newer, close Codex, then run `scripts/install-dream-skin.ps1` once. The installer preserves the user's native appearance settings, seeds the Yier & Bubu theme, copies the runtime to `%LOCALAPPDATA%\CodexDreamSkin\engine`, and creates launch/restore/tray shortcuts that do not depend on the source checkout.
...
- `assets/dream-reference.jpg`: pure 2560 × 1440 Yier & Bubu wallpaper seeded as the default and as a saved theme; it contains no Codex UI.
