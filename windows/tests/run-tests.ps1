...
try {
  ...
  if ($initialTheme.Theme.id -cne 'preset-yier-bubu-cozy' -or
    $initialTheme.Theme.name -cne '一二布布 · 暖暖编码' -or
    ...
    throw 'Default Windows theme did not seed the Yier Bubu wallpaper contract.'
  }
  $preseededThemes = @(Get-DreamSkinSavedThemes -StateRoot $themeStateRoot)
  if ($preseededThemes.Count -ne 1 -or
    $preseededThemes[0].Id -cne 'preset-yier-bubu-cozy' -or
    $preseededThemes[0].Name -cne '一二布布 · 暖暖编码') {
    throw 'Yier Bubu was not preseeded in the Windows saved-theme menu.'
  }
